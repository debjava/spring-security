...
import com.ddlab.rnd.spring.services.BankingServices;
...
public class BankResources extends BaseResource {
	...
	public TokenTransfer authenticate(@FormParam("username") String username,
		...
		return services.getToken(username, password);
	}
	...
	public AccountDetails getUser(@QueryParam("id") int id) {
		if (id == 0)
			throw createWebappException(new IllegalArgumentException(
					"Incorrect ID"));
